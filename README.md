...
- AI 配置：支持配置 OpenAI、Anthropic、Gemini 兼容接口，测试连接，并用于小说简介、标签、设定、角色和角色关系生成。
- 爬虫管理：支持创建抓取任务、站点规则、列表页批量建任务、任务重试、任务恢复和抓取结果入库。
- 系统管理：支持在管理页开启登录保护，并维护本地登录用户。
...
├── admin_routes.py        # 登录开关、登录会话和用户管理 API
...
进入首页后可以搜索小说名或作者，并按分类、标签、阅读状态筛选。卡片上提供详情、阅读、下载、编辑和删除操作；详情页会集中展示简介、标签、阅读进度、最近阅读时间、章节数、字数、文件路径、文件大小、文件检查状态、AI 角色分析和角色关系谱。左上角圆形勾选控件用于批量操作。打开 TXT 小说阅读时会自动恢复到上次阅读的章节和滚动位置，阅读器支持主题、字号、行高、正文宽度、段落间距、目录收起和沉浸模式。
...
- 测试 AI 对话连接。
- 在单本小说编辑时生成简介和标签。
- 在单本小说详情页提取世界观、地点、组织、规则体系、时间线、关键物品和术语等设定。
...
在“系统管理”中可以维护登录保护和本地用户：

- 默认不强制登录，升级旧版本后不会突然进不去。
- 开启登录前必须先创建至少一个启用的管理员用户。
- 登录开启后，除登录状态和登录接口外，其他数据接口都需要先登录。
- 支持新增用户、修改用户名/显示名称、重置密码、启用/停用、设置管理员和删除用户。
- 系统会阻止停用、降级或删除最后一个启用的管理员，避免把自己锁在门外。

如果需要指定服务端会话密钥，可以在启动前设置 `APP_SECRET_KEY`。未设置时，系统会在本地 `instance/app_secret.key` 中生成并复用一个随机密钥。
